...
def update_customer(request, pk):
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            messages.success(
                request, f"Customer #{customer_to_update.pk} edited successfully"
            )
            return redirect("website:home")
...
def product_list(request):
    ...
    if search_query:
        products = products.filter(
            ...
            )  # Не оптимально для highload. Потім почитати про SearchVector
        )
...
def product_detail(request, pk):
    ...
    return render(
        request, "website/product_detail.html", {"product": product, "form": form}
    )
...
def master_form(request):
    if request.method == "POST":
        ...
        )  # по дефолту стояв би і так 'items', бо в моделі related_name='items', але робимо explicitly
        del order_form.fields[
            "customer"
        ]  # щоб пройти валідацію не передавши користувача (він ще не створений)
        ...
        if customer_form.is_valid() and order_form.is_valid() and formset.is_valid():
            try:
                with transaction.atomic():
                    ...
                    messages.success(
                        request,
                        f"Customer #{customer.id} and Order #{order.id} created successfully",
                    )
                    return redirect("website:order_detail", pk=order.id)
    ...
    return render(
        request,
        "website/master_form.html",
        {"customer_form": customer_form, "order_form": order_form, "formset": formset},
    )
